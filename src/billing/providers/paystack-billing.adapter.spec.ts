...
describe('PaystackBillingAdapter', () => {
  ...
  beforeEach(() => {
    ...
    jest
      ...
      .mockResolvedValue({
        data: {
          data: {
            authorization_url: 'https://paystack.com/pay/ref',
            reference: 'ref-1',
          },
        },
      });
  ...
  it('initializes checkout with plan code and metadata', async () => {
    ...
    expect(body.callback_url).toBe(
      'https://joinjubily.com/billing/paystack/callback',
    );
  ...
  it('verifies callback transactions by reference', async () => {
    ...
    await expect(adapter.verifyTransaction('ref-1')).resolves.toEqual(
      expect.objectContaining({
    ...
    expect(axios.get).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        headers: { Authorization: 'Bearer sk_paystack' },
      }),
    );
  });
});
