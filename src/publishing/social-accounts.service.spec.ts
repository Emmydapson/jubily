...
describe('SocialAccountsService', () => {
  ...
  beforeEach(() => {
    ...
    process.env.TIKTOK_REDIRECT_URI =
      'https://api.joinjubily.com/api/auth/tiktok/callback';
    process.env.FACEBOOK_APP_ID = 'fb-app';
    process.env.FACEBOOK_REDIRECT_URI =
      'https://api.joinjubily.com/api/auth/facebook/callback';
    prisma = {
      socialAccount: {
        findMany: jest.fn(),
        findFirst: jest.fn(),
  ...
  it('returns friendly provider errors for unapproved TikTok and Meta publishing', async () => {
    ...
    await expect(
      service.publish({ workspaceId: 'workspace-1', provider: 'YOUTUBE' }),
