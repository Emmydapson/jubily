...
export class SocialOAuthController {
  ...
  async tiktokConnect(@Req() req: WorkspaceRequest, @ActiveWorkspace() workspace: { id: string }) {
  ...
  async tiktokCallback(@Res() res: Response, @Query('code') code?: string, @Query('state') state?: string) {
    if (!code) return res.redirect(this.frontendPublishingUrl({ error: 'TIKTOK_TOKEN_EXCHANGE_FAILED' }));
    const pending = await this.oauthStates.consume('workspace_tiktok', state);
    if (!pending?.workspaceId || !pending.userId) {
      return res.redirect(this.frontendPublishingUrl({ error: 'INVALID_CALLBACK_STATE' }));
    }
    try {
      await this.workspaces.requireMembership(pending.workspaceId, pending.userId, ['OWNER', 'ADMIN']);
      await this.accounts.handleTikTokCallback(pending.workspaceId, pending.userId, code);
      return res.redirect(this.frontendPublishingUrl({ connected: 'tiktok' }));
    } catch {
      return res.redirect(this.frontendPublishingUrl({ error: 'TIKTOK_CONNECTION_FAILED' }));
    }
  }
  ...
  async facebookConnect(@Req() req: WorkspaceRequest, @ActiveWorkspace() workspace: { id: string }) {
    if (!req.user?.userId) throw new UnauthorizedException('SaaS user token required');
    const state = await this.oauthStates.create({
  ...
  async facebookCallback(@Res() res: Response, @Query('code') code?: string, @Query('state') state?: string) {
    if (!code) return res.redirect(this.frontendPublishingUrl({ error: 'FACEBOOK_TOKEN_EXCHANGE_FAILED' }));
    ...
    try {
      await this.workspaces.requireMembership(pending.workspaceId, pending.userId, ['OWNER', 'ADMIN']);
      await this.accounts.handleFacebookCallback(pending.workspaceId, pending.userId, code);
      return res.redirect(this.frontendPublishingUrl({ connected: 'facebook' }));
    } catch {
      return res.redirect(this.frontendPublishingUrl({ error: 'FACEBOOK_CONNECTION_FAILED' }));
    }
  }
}
