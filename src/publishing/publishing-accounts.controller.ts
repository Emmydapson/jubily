...
export class PublishingAccountsController {
  ...
  @Get()
  @ApiOperation({ summary: 'List workspace publishing accounts without provider tokens' })
  ...
  @ApiOperation({ summary: 'Select default Facebook Page or Instagram business account' })
  select(
    @Param('id') id: string,
    @Body() body: { selectedPageId?: string | null; selectedInstagramBusinessAccountId?: string | null },
    @Req() req: WorkspaceRequest,
    @ActiveWorkspace() workspace: { id: string },
  ) {
    return this.accounts.selectAccount(workspace.id, id, body, req.user?.userId);
  }

  @Post(':id/disconnect')
  @WorkspaceRoles('OWNER', 'ADMIN')
  @ApiOperation({ summary: 'Disconnect a workspace publishing account without deleting publish history' })
  disconnect(@Param('id') id: string, @Req() req: WorkspaceRequest, @ActiveWorkspace() workspace: { id: string }) {
