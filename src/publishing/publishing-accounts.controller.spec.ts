...
describe('PublishingAccountsController', () => {
  ...
  beforeEach(() => {
    accounts = {
      listAccounts: jest
        .fn()
        .mockResolvedValue([{ id: 'account-1', provider: 'TIKTOK' }]),
      selectAccount: jest
        .fn()
        .mockResolvedValue({ id: 'account-1', selectedPageId: 'page-1' }),
      disconnectAccount: jest
        .fn()
        .mockResolvedValue({ id: 'account-1', status: 'DISCONNECTED' }),
    };
    controller = new PublishingAccountsController(accounts);
  });

  it('lists publishing accounts for the active workspace', async () => {
    await expect(controller.list({ id: 'workspace-1' })).resolves.toEqual([
      { id: 'account-1', provider: 'TIKTOK' },
    ]);
  ...
  it('selects a page/account scoped to the active workspace', async () => {
    ...
    expect(accounts.selectAccount).toHaveBeenCalledWith(
      'workspace-1',
      'account-1',
      { selectedPageId: 'page-1' },
      'user-1',
