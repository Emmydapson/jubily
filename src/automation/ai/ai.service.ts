...
- Every visualPrompt must include subject action, environment, emotional tone, product/category context, vertical 9:16 framing, lighting, realism, and "no text".
- Vary visual shot types: close-up, product screen, over-the-shoulder, before-and-after, proof/result, environment, CTA card.
...
- Return only JSON. No explanation.
  ...
  async rewriteScriptForQuality(params: {
    ...
    if (this.aiMode === 'mock') {
      ...
      parsed.hook =
        parsed.hook || `Most people miss this about ${params.topic}.`;
      ...
      parsed.scenes =
        parsed.scenes.length >= 5
          ...
          : [
              ...
              {
                narration:
                  'Do it consistently and your routine starts feeling easier.',
      ...
      while (parsed.scenes.length < 8) {
        parsed.scenes.push({
          narration:
            'Add one small repeatable step so the routine feels realistic on a busy day.',
      ...
      parsed.scenes = parsed.scenes.map((scene) => ({
        ...scene,
        seconds: Math.max(2.5, Math.min(5, Number(scene.seconds || 4))),
      }));
      return JSON.stringify(parsed);
    }
...
- Target 30-60 seconds for short-form, with 40-50 seconds ideal.
- Use 10-16 short visual beats when needed for retention.
- Each scene must have "seconds" between 2.5 and 5.
- Split long ideas into multiple visual beats instead of holding one image.
- The sum of all scene "seconds" should usually be between 30 and 60.
