...
export class ShotstackService {
  ...
  const renderMode = (process.env.RENDER_MODE || 'live').toLowerCase(); // live | mock
  if (renderMode === 'mock') {
    // ✅ bypass Shotstack + TTS completely
  ...
  for (const scene of scenes) {
    ...
    bgClips.push({
      asset: { type: 'image', src: this.unsplashImage(scene.visualPrompt) },
      start,
      length,
      effect: 'zoomIn',
    });
