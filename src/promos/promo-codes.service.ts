...
export class PromoCodesService {
  ...
  private parseDate(value?: string | Date | null) {
    ...
    if (Number.isNaN(date.getTime()))
      throw new BadRequestException('Invalid promo code date');
    return date;
  }
  ...
  private normalizeCountries(codes?: string[] | null) {
    return (codes || [])
      .map((code) => this.normalizeCountry(code))
      .filter(Boolean);
  }

  private assertDiscount(
    dto: Pick<CreatePromoCodeDto, 'discountType' | 'discountValue'>,
  ) {
    ...
    if (dto.discountValue == null || Number(dto.discountValue) <= 0) {
      throw new BadRequestException(
        'discountValue is required for discount promo codes',
      );
    }
    if (
  ...
  private assertAdminEnums(dto: Partial<CreatePromoCodeDto>) {
    if (
      ...
      !Object.values(PromoDiscountType).includes(dto.discountType)
    ) {
      throw new BadRequestException(
        'discountType must be PERCENTAGE, FIXED, or NONE',
      );
    }
    if (
      dto.discountDuration != null &&
      dto.discountDuration !== PromoDiscountDuration.ONE_TIME
    ) {
      throw new BadRequestException('discountDuration must be ONE_TIME');
    }
    if (
      dto.appliesToPlans != null &&
      !Object.values(PromoAppliesToPlan).includes(dto.appliesToPlans)
    ) {
      throw new BadRequestException(
        'appliesToPlans must be PRO, PREMIUM, or ALL',
      );
    }
    if (
      dto.regionScope != null &&
      !Object.values(PromoRegionScope).includes(dto.regionScope)
    ) {
      throw new BadRequestException(
    ...
    if (
      dto.paystackDiscountMode != null &&
      !Object.values(PaystackDiscountMode).includes(dto.paystackDiscountMode)
    ) {
      throw new BadRequestException(
  ...
  private assertRegionConfig(
    ...
    const scope =
      dto.regionScope ?? existing?.regionScope ?? PromoRegionScope.ALL;
    if (
      dto.allowedCountries !== undefined &&
      !Array.isArray(dto.allowedCountries)
    ) {
      throw new BadRequestException(
    ...
    const countries =
      dto.allowedCountries !== undefined
        ? dto.allowedCountries
        : existing?.allowedCountries;
    const normalized = this.normalizeCountries(countries);
    if (
      (countries || []).length !== normalized.length ||
      normalized.some((code) => !/^[A-Z]{2}$/.test(code))
    ) {
      throw new BadRequestException(
    ...
    if (new Set(normalized).size !== normalized.length) {
      throw new BadRequestException(
        'allowedCountries must not contain duplicate country codes',
      );
    }
    if (
      scope === PromoRegionScope.CUSTOM_COUNTRIES &&
      normalized.length === 0
    ) {
      throw new BadRequestException(
        'allowedCountries is required when regionScope is CUSTOM_COUNTRIES',
      );
    }
  }

  private assertAdminPayload(
    dto: Partial<CreatePromoCodeDto>,
    existing?: {
    ...
    const discountType =
      dto.discountType ?? existing?.discountType ?? PromoDiscountType.NONE;
    const discountValue =
      dto.discountValue ?? existing?.discountValue ?? undefined;
    this.assertDiscount({ discountType, discountValue });
    this.assertRegionConfig(dto, existing);
    this.assertProviderConfig(dto);
  }

  private defaultPaystackDiscountMode(
    discountType?: PromoDiscountType | null,
    paystackDiscountMode?: PaystackDiscountMode | null,
  ) {
  ...
  private assertProviderConfig(
    dto: Pick<CreatePromoCodeDto, 'stripePromotionCodeId' | 'stripeCouponId'>,
  ) {
    const stripePromotionCodeId = this.normalizeOptional(
      dto.stripePromotionCodeId,
    );
    ...
    if (stripePromotionCodeId && !stripePromotionCodeId.startsWith('promo_')) {
      throw new BadRequestException(
        'stripePromotionCodeId must start with "promo_"',
      );
    }
  ...
  private assertApplies(
    code: { appliesToPlans: PromoAppliesToPlan },
    plan?: Plan | null,
  ) {
    if (
      !plan ||
      plan === Plan.FREE ||
      code.appliesToPlans === PromoAppliesToPlan.ALL
    )
      return;
    if (code.appliesToPlans !== plan)
      throw new BadRequestException('Promo code does not apply to this plan');
  }

  private assertRegion(
    code: {
      regionScope?: PromoRegionScope | null;
      allowedCountries?: string[] | null;
    },
    ...
    if (!country)
      throw new BadRequestException('Country is required for this promo code');
  ...
  private async findUsableByCode(
    code: string,
    plan?: Plan | null,
    countryCode?: string | null,
  ) {
  ...
  private selectProvider(
    ...
    return this.africanCountryCodes.has(this.normalizeCountry(countryCode))
  ...
  private pricingPreview(
    ...
    const plan = input.plan && input.plan !== Plan.FREE ? input.plan : Plan.PRO;
    ...
    const discountAmount =
      promo.discountType === PromoDiscountType.PERCENTAGE
        ? Math.round(
            (originalAmount * Math.min(100, Math.max(0, discountValue))) / 100,
          )
    ...
    return {
      ...
      renewalNotice:
        'Discount applies to this payment only. Renewals continue at the standard price.',
    };
  }

  private async assertNotRedeemed(
    promoCodeId: string,
    userId?: string | null,
    ...
    if (existing)
      throw new BadRequestException(
        'This promo code has already been used on this account.',
      );
  }
  ...
  private publicPromo(
    ...
    const stripeDiscountConfigured = Boolean(
      this.normalizeOptional(promo.stripePromotionCodeId),
    );
    const paystackDiscountMode =
      promo.paystackDiscountMode ?? PaystackDiscountMode.UNSUPPORTED;
    let providerSupported = true;
    let providerError: string | null = null;
    if (
      isDiscount &&
      preview.provider === BillingProvider.STRIPE &&
      !stripeDiscountConfigured
    ) {
      providerSupported = false;
      providerError =
        'This promo code is not configured for Stripe checkout yet.';
    }
    if (
      isDiscount &&
      preview.provider === BillingProvider.PAYSTACK &&
      paystackDiscountMode === PaystackDiscountMode.UNSUPPORTED
    ) {
      providerSupported = false;
      providerError =
        'Paystack one-time subscription discounts are not yet supported.';
    }
    if (
      isDiscount &&
      preview.provider === BillingProvider.PAYSTACK &&
      ...
      providerError =
        'This Paystack promo code is tracking-only and will not change the checkout price.';
    }
    return {
      valid: true,
      code: promo.code,
      ...
      planApplies:
        !input.plan ||
        promo.appliesToPlans === PromoAppliesToPlan.ALL ||
        promo.appliesToPlans === input.plan,
  ...
  async create(dto: CreatePromoCodeDto, adminId?: string | null) {
    ...
    if (!influencerName)
      throw new BadRequestException('influencerName is required');
    ...
    try {
      return await this.prisma.promoCode.create({
        data: {
          code,
          influencerName,
          influencerEmail: dto.influencerEmail
            ? String(dto.influencerEmail).trim().toLowerCase()
            : null,
          ...
          stripePromotionCodeId: this.normalizeOptional(
            dto.stripePromotionCodeId,
          ),
          stripeCouponId: this.normalizeOptional(dto.stripeCouponId),
          paystackDiscountMode: this.defaultPaystackDiscountMode(
            discountType,
            dto.paystackDiscountMode,
          ),
  ...
  async update(id: string, dto: UpdatePromoCodeDto) {
    ...
    const data: any = {
      influencerName:
        dto.influencerName != null
          ? String(dto.influencerName).trim()
          : undefined,
      influencerEmail:
        dto.influencerEmail != null
          ? String(dto.influencerEmail).trim().toLowerCase()
          : undefined,
      description:
        dto.description != null ? String(dto.description).trim() : undefined,
      discountType: dto.discountType,
      discountValue:
        dto.discountType === PromoDiscountType.NONE ? null : dto.discountValue,
      discountDuration: PromoDiscountDuration.ONE_TIME,
      appliesToPlans: dto.appliesToPlans,
      regionScope: dto.regionScope,
      allowedCountries:
        dto.allowedCountries !== undefined
          ? this.normalizeCountries(dto.allowedCountries)
          : undefined,
      stripePromotionCodeId:
        dto.stripePromotionCodeId !== undefined
          ? this.normalizeOptional(dto.stripePromotionCodeId)
          : undefined,
      stripeCouponId:
        dto.stripeCouponId !== undefined
          ? this.normalizeOptional(dto.stripeCouponId)
          : undefined,
      paystackDiscountMode:
        dto.paystackDiscountMode !== undefined || dto.discountType !== undefined
          ? this.defaultPaystackDiscountMode(
              nextDiscountType,
              dto.paystackDiscountMode,
            )
          : undefined,
      ...
      startsAt:
        dto.startsAt !== undefined ? this.parseDate(dto.startsAt) : undefined,
      expiresAt:
        dto.expiresAt !== undefined ? this.parseDate(dto.expiresAt) : undefined,
      isActive: dto.isActive,
    };
    if (dto.code != null) data.code = this.normalizeCode(dto.code);
    if (dto.code != null && !data.code)
      throw new BadRequestException('code is required');
    if (dto.influencerName != null && !data.influencerName)
      throw new BadRequestException('influencerName is required');
    try {
      ...
      if (error?.code === 'P2002')
        throw new ConflictException(
          `Promo code "${data.code ?? existing.code}" already exists`,
        );
      throw error;
    }
  }
  ...
  async recordCheckoutStarted(input: CheckoutAttributionInput) {
    const promo = await this.findUsableByCode(
      input.code,
      input.plan,
      input.countryCode,
    );
    ...
    const paystackDiscountMode =
      (promo as any).paystackDiscountMode ?? PaystackDiscountMode.UNSUPPORTED;
    if (
      promo.discountType !== PromoDiscountType.NONE &&
  ...
  async markSubscribed(input: SubscribeInput) {
    ...
    const data = {
      ...
      discountDuration:
        (input.discountDuration as any) || PromoDiscountDuration.ONE_TIME,
      redeemedAt: new Date(),
      status: PromoAttributionStatus.SUBSCRIBED,
    };

    const existing = promoAttributionId
      ? await this.prisma.promoAttribution.findUnique({
          where: { id: promoAttributionId },
        })
      : await this.prisma.promoAttribution.findFirst({
          where: {
            ...
            status: {
              in: [
                PromoAttributionStatus.CHECKOUT_STARTED,
                PromoAttributionStatus.SIGNUP,
              ],
            },
          },
    ...
    const updated = await this.prisma.promoAttribution
  ...
  async performance(id: string) {
    ...
    const count = (status: PromoAttributionStatus) =>
      attributions.filter((item) => item.status === status).length;
    const subscribed = attributions.filter(
      (item) => item.status === PromoAttributionStatus.SUBSCRIBED,
    );
    const revenue = subscribed.reduce(
      (sum, item) => sum + Number(item.finalAmount ?? item.amount ?? 0),
      0,
    );
    const revenueBeforeDiscount = subscribed.reduce(
      (sum, item) => sum + Number(item.originalAmount ?? item.amount ?? 0),
      0,
    );
    const discountTotal = subscribed.reduce(
    ...
    for (const item of subscribed) {
      ...
      if (item.provider)
        revenueByProvider[item.provider] =
          (revenueByProvider[item.provider] || 0) + amount;
      if (item.plan)
        revenueByPlan[item.plan] = (revenueByPlan[item.plan] || 0) + amount;
    }
    ...
    return {
      ...
      conversionRate: checkoutStarts
        ? successfulSubscriptions / checkoutStarts
        : 0,
      ...
      stripeConfigured: {
        ...
        count: this.normalizeOptional((promo as any).stripePromotionCodeId)
          ? 1
          : 0,
      },
      paystackDiscountMode:
      ...
      redemptionUniqueness: {
        ...
        uniqueWorkspaces: new Set(
          subscribed.map((item) => item.workspaceId).filter(Boolean),
        ).size,
      },
      revenueByProvider,
      revenueByPlan,
