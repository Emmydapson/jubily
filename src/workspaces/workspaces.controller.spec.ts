...
describe('WorkspacesController YouTube OAuth flow', () => {
  ...
  beforeEach(() => {
    ...
    youtube = {
      getCustomerAuthUrl: jest.fn(
        (state: string) =>
          `https://accounts.google.com/o/oauth2/v2/auth?redirect_uri=${encodeURIComponent('https://api.joinjubily.com/api/auth/youtube/callback')}&state=${encodeURIComponent(state)}`,
      ),
      handleWorkspaceAuthCallback: jest
        .fn()
        .mockResolvedValue({ connected: true, channelId: 'UC_WORKSPACE' }),
    };
    oauthStates = {
  ...
  it('customer connect returns a Google OAuth URL with the global customer callback redirect', async () => {
    ...
    expect(youtube.getCustomerAuthUrl).toHaveBeenCalledWith(
      'workspace-state-1',
    );
  ...
  it('customer connect returns a friendly config error when OAuth is missing', async () => {
    ...
    await expect(
      controller.youtubeConnect(req as never, workspace),
